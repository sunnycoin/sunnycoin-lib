...
function createEscrowCreationTransaction(account: string,
  ...
  const txJSON: any = {
    ...
    Amount: sunToDrops(payment.amount)
  }
  ...
  if (payment.allowCancelAfter !== undefined) {
    txJSON.CancelAfter = iso8601ToSunnycoinTime(payment.allowCancelAfter)
  }
  if (payment.allowExecuteAfter !== undefined) {
    txJSON.FinishAfter = iso8601ToSunnycoinTime(payment.allowExecuteAfter)
  }
