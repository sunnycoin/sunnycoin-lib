...
import {SunnycoindError, DisconnectedError, NotConnectedError,
  TimeoutError, ResponseFormatError, ConnectionError,
  SunnycoindNotInitializedError} from './errors'
  ...
  _onOpen() {
    ...
    return this.request(request).then((data: any) => {
      if (_.isEmpty(data) || !data.ledger_index) {
        // sunnycoind instance doesn't have validated ledgers
        return this._disconnect(false).then(() => {
          throw new SunnycoindNotInitializedError('Sunnycoind not initialized')
        })
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(eventName, response => {
        if (response.status === 'error') {
          _reject(new SunnycoindError(response.error, response))
