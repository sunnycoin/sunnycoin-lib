...
var uglifyOptions = {
  mangle: {
    reserved: ['_', 'SunnycoinError', 'SunnycoindError', 'UnexpectedError',
...
function getWebpackConfig(extension, overrides) {
  ...
  let defaults = {
    ...
    output: {
      ...
      filename: ['sunnycoin-', extension].join(pkg.version)
    },
    plugins: [
...
function createBuildLink(callback) {
  return function(err, res) {
    createLink('./build/sunnycoin-' + pkg.version + '.js',
      './build/sunnycoin-latest.js');
    callback(err, res);
  };
}
...
gulp.task('build-min', function(callback) {
  ...
  webpack(webpackConfig, function() {
    createLink('./build/sunnycoin-' + pkg.version + '-min.js',
      './build/sunnycoin-latest-min.js');
    callback();
  });
});
...
gulp.task('bower-build', ['build'], function() {
  return gulp.src(['./build/sunnycoin-', '.js'].join(pkg.version))
  .pipe(rename('sunnycoin.js'))
  .pipe(gulp.dest('./dist/bower'));
});

gulp.task('bower-build-min', ['build-min'], function() {
  return gulp.src(['./build/sunnycoin-', '-min.js'].join(pkg.version))
  .pipe(rename('sunnycoin-min.js'))
  .pipe(gulp.dest('./dist/bower'));
});

gulp.task('bower-build-debug', ['build-debug'], function() {
  return gulp.src(['./build/sunnycoin-', '-debug.js'].join(pkg.version))
  .pipe(rename('sunnycoin-debug.js'))
  .pipe(gulp.dest('./dist/bower'));
});
