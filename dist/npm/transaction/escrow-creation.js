...
function createEscrowCreationTransaction(account, payment) {
    const txJSON = {
        ...
        Amount: common_1.sunToDrops(payment.amount)
    };
    if (payment.condition !== undefined) {
        txJSON.Condition = payment.condition;
    }
    if (payment.allowCancelAfter !== undefined) {
        txJSON.CancelAfter = common_1.iso8601ToSunnycoinTime(payment.allowCancelAfter);
    }
    if (payment.allowExecuteAfter !== undefined) {
        txJSON.FinishAfter = common_1.iso8601ToSunnycoinTime(payment.allowExecuteAfter);
    }
