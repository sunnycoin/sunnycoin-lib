...
function adjustQualityForSUN(quality, takerGetsCurrency, takerPaysCurrency) {
    // quality = takerPays.value/takerGets.value
    // using drops (1e-6 SUN) for SUN values
    const numeratorShift = (takerPaysCurrency === 'SUN' ? -6 : 0);
    const denominatorShift = (takerGetsCurrency === 'SUN' ? -6 : 0);
    const shift = numeratorShift - denominatorShift;
    return shift === 0 ? quality :
        (new bignumber_js_1.default(quality)).shift(shift).toString();
}
exports.adjustQualityForSUN = adjustQualityForSUN;
...
function parseTimestamp(sunnycoinTime) {
    if (typeof sunnycoinTime !== 'number') {
        return undefined;
    }
    return common.sunnycoinTimeToISO8601(sunnycoinTime);
}
